                             ...
                             const MessageList &containedMessages)
{
  ...
  /////////////////////////////
  //  ADD INTERNAL QKD COMMAND TAG 
  ////////////////////////////

  QKDCommandTag qkdCommandTag; 
  packet->RemovePacketTag(qkdCommandTag);
  qkdCommandTag.SetCommand ('R');
  qkdCommandTag.SetRoutingProtocolNumber(OLSR_PACKET_HEADER_PROTOCOL_NUMBER);
  packet->AddPacketTag (qkdCommandTag);
...
RoutingProtocol::SendQueuedMessages ()
{
  ...
  packet = m_ipv4->GetObject<Node> ()->GetObject<QKDManager> ()->MarkEncrypt (packet);
  int numMessages = 0;
  ...
  for (std::vector<olsr::MessageHeader>::const_iterator message = m_queuedMessages.begin ();
       ...
       message++)
    {
      ...
      p = m_ipv4->GetObject<Node> ()->GetObject<QKDManager> ()->MarkEncrypt (p);
      p->AddHeader (*message);
      packet->AddAtEnd (p);
  ...
  packet->RemovePacketTag(qkdCommandTag);
  qkdCommandTag.SetCommand ('R');
  qkdCommandTag.SetRoutingProtocolNumber(OLSR_MESSAGE_HEADER_PROTOCOL_NUMBER);
  packet->AddPacketTag (qkdCommandTag);
