...
PointToPointNetDevice::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PointToPointNetDevice")
    ...
    .AddTraceSource ("MacRx", 
                     ...
                     "ns3::Packet::TracedCallback")
    .AddAttribute   ("SniffFromNetDevice", "Indicates whether a sniffing from this netDevice is enabled.",
                    BooleanValue (true),
                    MakeBooleanAccessor (&PointToPointNetDevice::m_sniffingFromNetDevice),
                    MakeBooleanChecker ())
#if 0
...
void 
PointToPointNetDevice::SetSniffPacketFromDevice( bool value){

  NS_LOG_FUNCTION(this << value);
  m_sniffingFromNetDevice = value;  
  
}

void
PointToPointNetDevice::SniffPacket(Ptr<const Packet> p){

  NS_LOG_FUNCTION( this << p << p->GetUid() << p->GetSize() ); 
  /*
  std::cout << "--------PointToPointNetDevice::SniffPacket-----------\n";   
  std::cout << p->GetUid() << "\t" << p->GetSize() << "\n";
  p->Print(std::cout);
  std::cout << "\n\n";
  std::cout << "--------End of PointToPointNetDevice::SniffPacket-----------\n";
  */
  m_snifferTrace (p);
...
bool
PointToPointNetDevice::ShouldSniffPacketFromDevice(){

  return m_sniffingFromNetDevice;
}
...
PointToPointNetDevice::SetNode (Ptr<Node> node)
{
  ...
  if(m_node->GetObject<QKDL4TrafficControlLayer> () == 0 || m_node->GetObject<QKDManager> () == 0 )
    m_sniffingFromNetDevice = true;
  else
    m_sniffingFromNetDevice = false;
}
